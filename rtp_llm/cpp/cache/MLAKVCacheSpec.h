...
#if USING_ROCM
        // ROCm sparse MLA uses aiter's FP8 KV cache layout:
        // 576 bytes/token (512 fp8 nope + 64 fp8 rope), per-tensor scale passed separately.
        // bf16 MLA layout is unchanged (no_pe + rope).
        spec->elems_per_token = no_pe + rope;
#else
        // rtp-llm native fp8 layout: 656 bytes/token with interleaved per-128-group
        // fp32 scales and bf16 rope. bf16 MLA is unchanged (no_pe + rope).
